  ...
  export const saveContratacion = async (data: ContratacionData): Promise<boolean> => {
    try {
      const response = await fetch(`http://${ipFetch}:3000/contrataciones`, {
        method: 'POST',
        headers: {
  ...
  export const updateContratacion = async (data: ContratacionData): Promise<boolean> => {
    try {
      const response = await fetch(`http://${ipFetch}:3000/contrataciones/update`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  export const deleteContratacion = async (contractId: number): Promise<boolean> => {
    try {
      const response = await fetch(`http://${ipFetch}:3000/contrataciones/${contractId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
